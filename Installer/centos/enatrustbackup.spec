...
Source0:	%{namee}-%{_builddate}.tar.bz2
...
install -p  Installer/debian/duplicati.png %{buildroot}%{_datadir}/pixmaps/
...
/bin/bash Installer/centos/%{namer}-install-recursive.sh "Duplicati/Server/webroot/" "%{buildroot}%{_exec_prefix}/lib/%{namer}/webroot/"

/bin/bash Installer/centos/%{namer}-install-recursive.sh "Duplicati/GUI/Duplicati.GUI.TrayIcon/bin/Release/licenses/" "%{buildroot}%{_exec_prefix}/lib/%{namer}/licenses/"
...
# Install the service:
install -p -D -m 755 Installer/centos/%{namer}.service %{_unitdir}
install -p -D -m 644 Installer/centos/%{namer}.default %{_sysconfdir}/sysconfig/
